-- VERIDIS - Optimized dashboard query
-- psql-ready example:
...
-- La CTE rapport_scope garde le contrôle d'accès organisation_id sur une seule ligne rapport.
-- La CTE reponses_filtrees force le filtrage tenant_id + rapport_id + statut avant les jointures de dimensions.
-- Le tri reste volontairement en mémoire: sur un dashboard rapport, le jeu attendu est petit après filtrage.
